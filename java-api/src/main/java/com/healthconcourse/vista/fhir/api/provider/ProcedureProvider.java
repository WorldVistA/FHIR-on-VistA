...
public class ProcedureProvider extends AbstractJaxRsResourceProvider<Procedure> {

    private static final Logger LOG = LoggerFactory.getLogger(ProcedureProvider.class);
    private final PatientService service;

    @Autowired
    public ProcedureProvider(VistaData data) {
    ...
    @Search
    public List<Procedure> search(@OptionalParam(name = Procedure.SP_SUBJECT) StringParam subjectId,
                                  @OptionalParam(name = Procedure.SP_PATIENT) StringParam patientId) {
        if (subjectId != null) {
            return this.findByPatient(subjectId.getValue());
        } else if (patientId != null) {
            return this.findByPatient(patientId.getValue());
        } else {
            throw new InvalidRequestException("No Procedure search parameters specified");
        }
    }

    private List<Procedure> findByPatient(String id) {
        List<Procedure> results = service.getProcedures(id);

        if(results.isEmpty()) {
            String message = "No Procedure records found for patient: " + id;
            LOG.info(message);
