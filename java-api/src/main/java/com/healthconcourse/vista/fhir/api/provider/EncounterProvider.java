...
public class EncounterProvider extends AbstractJaxRsResourceProvider<Encounter> {

    private static final Logger LOG = LoggerFactory.getLogger(EncounterProvider.class);
    private final PatientService service;

    @Autowired
    public EncounterProvider(VistaData data) {
    ...
    @Search()
    public List<Encounter> search(@OptionalParam(name = Encounter.SP_SUBJECT) StringParam subjectId,
                                  @OptionalParam(name = Encounter.SP_PATIENT) StringParam patientId) {
        if (subjectId != null) {
    ...
    private List<Encounter> findByPatient(String id) {
        List<Encounter> results = service.getEncountersForPatient(id);

        if(results.isEmpty()) {
            String message = "No Encounter records found for patient: " + id;
            LOG.info(message);
