...
public class MedicationDispenseProvider extends AbstractJaxRsResourceProvider<MedicationDispense> {

    private static final Logger LOG = LoggerFactory.getLogger(MedicationDispenseProvider.class);
    private final PatientService service;

    @Autowired
    public MedicationDispenseProvider(VistaData data) {
    ...
    @Search()
    public List<MedicationDispense> search(@OptionalParam(name = MedicationDispense.SP_SUBJECT) StringParam subjectId,
                                           @OptionalParam(name = MedicationDispense.SP_PATIENT) StringParam patientId) {
        if (subjectId != null) {
    ...
    private List<MedicationDispense> findByPatient(String id) {
        List<MedicationDispense> results = service.getMedicationDispense(id);

        if(results.isEmpty()) {
            String message = "No MedicationDispense records found for patient: " + id;
            LOG.info(message);
