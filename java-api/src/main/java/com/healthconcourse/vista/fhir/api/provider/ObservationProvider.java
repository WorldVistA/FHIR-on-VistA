...
public class ObservationProvider extends AbstractJaxRsResourceProvider<Observation> {
    ...
    @Search
    public List<Observation> searchBySubject(@OptionalParam(name = Condition.SP_SUBJECT) StringParam subjectId,
    ...
    private List<Observation> findByPatient(String id) {
        List<Observation> results = service.getObservationsByIcn(id);

        if(results.isEmpty()) {
            String message = "No observation found for patient: " + id;
            LOG.info(message);
