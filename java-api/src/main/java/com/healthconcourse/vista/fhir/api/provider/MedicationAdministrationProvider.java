...
public class MedicationAdministrationProvider extends AbstractJaxRsResourceProvider<MedicationAdministration> {

    private static final Logger LOG = LoggerFactory.getLogger(MedicationAdministrationProvider.class);
    private final PatientService service;

    @Autowired
    public MedicationAdministrationProvider(VistaData data) {
    ...
    @Search()
    public List<MedicationAdministration> search(@OptionalParam(name = MedicationAdministration.SP_SUBJECT) StringParam subjectId,
                                           @OptionalParam(name = MedicationAdministration.SP_PATIENT) StringParam patientId) {
        if (subjectId != null) {
    ...
    private List<MedicationAdministration> findByPatient(String id) {
        List<MedicationAdministration> results = service.getMMedicationAdministration(id);

        if(results.isEmpty()) {
            String message = "No MedicationAdministration records found for patient: " + id;
            LOG.info(message);
