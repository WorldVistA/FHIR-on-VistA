...
public class DiagnosticReportProvider extends AbstractJaxRsResourceProvider<DiagnosticReport> {

    private static final Logger LOG = LoggerFactory.getLogger(DiagnosticReportProvider.class);
    private final PatientService service;

    @Autowired
    public DiagnosticReportProvider(VistaData data) {
    ...
    @Search()
    public List<DiagnosticReport> search(@OptionalParam(name = DiagnosticReport.SP_SUBJECT) StringParam subjectId,
                                         @OptionalParam(name = DiagnosticReport.SP_PATIENT) StringParam patientId) {
        if (subjectId != null) {
    ...
    private List<DiagnosticReport> findByPatient(String id) {
        List<DiagnosticReport> results = service.getDiagnosticReport(id);

        if(results.isEmpty()) {
            String message = "No DiagnosticReport records found for patient: " + id;
            LOG.info(message);
