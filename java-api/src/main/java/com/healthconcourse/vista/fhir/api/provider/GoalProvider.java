...
public class GoalProvider extends AbstractJaxRsResourceProvider<Goal> {

    private static final Logger LOG = LoggerFactory.getLogger(GoalProvider.class);
    private final PatientService service;

    @Autowired
    public GoalProvider(VistaData data) {
    ...
    @Search()
    public List<Goal> search(@OptionalParam(name = Goal.SP_SUBJECT) StringParam subjectId,
                             @OptionalParam(name = Goal.SP_PATIENT) StringParam patientId) {
        if (subjectId != null) {
    ...
    private List<Goal> findByPatient(String id) {
        List<Goal> results = service.getGoal(id);

        if(results.isEmpty()) {
            String message = "No goal records found for patient: " + id;
            LOG.info(message);
