...
public class ImmunizationProvider extends AbstractJaxRsResourceProvider<Immunization> {
    ...
    @Autowired
    public ImmunizationProvider(VistaData data) {
    ...
    @Search()
    public List<Immunization> search(@OptionalParam(name = Immunization.SP_PATIENT) StringParam patientId) {
        if (patientId != null) {
    ...
    private List<Immunization> findByPatient(String id) {
        List<Immunization> results = service.getImmunizationsByIcn(id);

        if(results.isEmpty()) {
            String message = "No immunization records found for patient: " + id;
            LOG.info(message);
